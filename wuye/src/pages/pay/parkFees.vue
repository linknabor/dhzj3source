		...
		methods:{
			...
			carLoadBottom(){//停车缴费，上拉加载数据
			  ...
			  pay(list,allPrice,allselect){//第一个参数 账单数组，第二个参数 总价 第三个参数 是否全选,所有参数 string
		  		...
		  		//------
		  		vm.str = vm.config.wuye_zhifu.payment;
		  		let url = vm.str + "?#/?billIds="+bills+"&stmtId="+vm.stmtId+"&payAddr="+escape(pay_addr)+"&totalPrice="+vm[allPrice]+"&reduceMode="+vm.reduceMode;
