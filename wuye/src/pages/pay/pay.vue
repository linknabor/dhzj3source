...
export default {
   ...
   methods: {
		...
		queryLoadBottom() {
			...
			vm.$refs.loadmore3.onBottomLoaded();
        ...
        pay(list,allPrice,allselect) {
			...
			let str = vm.config.wuye_zhifu.payment;
	  		let url = str +"billIds="+bills+"&stmtId="+vm.stmtId+"&payAddr="+escape(pay_addr)+"&totalPrice="+vm[allPrice]+"&reduceMode="+vm.reduceMode;
